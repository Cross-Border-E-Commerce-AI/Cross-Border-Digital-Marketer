...
- Given the dataset consists of $N$ images, with each represented by an embedding vector $\text{z}_{i} \in \mathbb{R}^D$, and the covariates $\text{x}_{i} \in \mathbb{R}^P$ influence the prevalence of visual topics. Model each image embedding $\text{z}_{i}$ as normally distributed and generated by a mixture of $K$ topics:
